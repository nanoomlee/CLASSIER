...
int perturbations_init(
  ...
  if (pba->has_ncdm == _TRUE_) {
    ...
    /* if there is no ncdm, we will not need the ncdmfft method. */
    ppt->has_ncdmfft = _FALSE_;
  }
 ...
 * @param pth Input/Output: pointer to ncdmfft workspace
 * @return the error status
 */
...
int perturbations_indices(
  ...
  /** - source flags when using ncdmfft */
  if(ppt->has_ncdmfft){
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_vectors_) {
      ...
      */
      ...
      if (ppt->has_ncdmfft == _TRUE_) {
        class_stop(ppt->error_message,"ncdmfft method not coded yet for vectors");
      }
    ...
    if (_tensors_) {
      ...
      */
      ...
      if (ppt->has_ncdmfft == _TRUE_) {
        class_stop(ppt->error_message,"ncdmfft method not coded yet for tensors");
      }
...
int perturbations_timesampling_for_sources(
  ...
  */
  ...
  /** - if we use the ncdmfft acceleration we need to find an additional timesampling */
  if (ppt->has_ncdmfft) {
    ...
    int index_nq=0;
    double chi_today_qmax=pvecback[pba->index_bg_chi_ncdmfft+index_nq], chi_today_qmin=pvecback[pba->index_bg_chi_ncdmfft+index_nq];
    for (int index_n=0;index_n<pba->N_ncdm;index_n++){
      chi_today_qmin = fmin(chi_today_qmin,pvecback[pba->index_bg_chi_ncdmfft+index_nq]);
      for (int index_q=0;index_q<pba->q_size_ncdm[index_n];index_q++){
        chi_today_qmax = fmax(chi_today_qmax, pvecback[pba->index_bg_chi_ncdmfft + index_nq]);
        index_nq += 1;
      }
    }

    /** - we need to determine the size of the maximal ncdmfft convolution. This will be also the size in which the kernel is evaluated */
...
#pragma omp parallel
  {
    ...
    ppr->N_G2 = 80000;
    if (k_max<=50) ppr->N_G2 = 40000;
    if (k_max<=5) ppr->N_G2 = 25000;
    if (k_max<=1) ppr->N_G2 = 5000;
    if (k_max<=0.1) ppr->N_G2 = 1000;

	if (ppr->N_G2%2==0) ppr->N_G2++;
	ppt->N_G2_freq_max = ppr->N_G2*10+1;

    ppt->xi_G1 = (double *) malloc(sizeof(double) * ppr->N_G1);
    ppt->w8_G1 = (double *) malloc(sizeof(double) * ppr->N_G1);
    quadrature_gauss_legendre (&ppt->xi_G1[1], &ppt->w8_G1[1], ppr->N_G1-2, 1e-9, ppt->error_message);
    ppt->xi_G1[0] = -1.; ppt->xi_G1[ppr->N_G1-1] = 1.;
    ppt->w8_G1[0] = 0.; ppt->w8_G1[ppr->N_G1-1] = 0.;
    
    class_alloc(ppt->xi_G2,sizeof(double *)*5,ppt->error_message);
    class_alloc(ppt->w8_G2,sizeof(double *)*5,ppt->error_message);
    int N_G2_kbin;
    for (int i=0;i<5;i++){
      if (i==0) N_G2_kbin = 1000;
      else if (i==1) N_G2_kbin = 5000;
      else if (i==2) N_G2_kbin = 25000;
      else if (i==3) N_G2_kbin = 40000;
      else if (i==4) N_G2_kbin = 80000;
      ppt->xi_G2[i] = (double *) malloc(sizeof(double) * N_G2_kbin);
      ppt->w8_G2[i] = (double *) malloc(sizeof(double) * N_G2_kbin);

      /*quadrature_gauss_legendre (&ppt->xi_G2[i][1], &ppt->w8_G2[i][1], N_G2_kbin-2, 1e-9, ppt->error_message);
      ppt->xi_G2[i][0] = -1.; ppt->xi_G2[i][N_G2_kbin-1] = 1.;
      ppt->w8_G2[i][0] = 0.; ppt->w8_G2[i][N_G2_kbin-1] = 0.;

      char file2[200];
      sprintf(file2, "GL/GL_%d.dat",N_G2_kbin);
      FILE *fout2;
      fout2 = fopen(file2, "a");
      for (int j = 0; j < N_G2_kbin; j++) fprintf(fout2,"%.12e %.12e\n", ppt->xi_G2[i][j], ppt->w8_G2[i][j]);
      fclose(fout2);*/
      ...
      for (int j = 0; j < N_G2_kbin; j++) {
        ...
        fscanf(fA,"%le", &(ppt->w8_G2[i][j]));
      }
      fclose(fA);

	  for (int j=0;j<N_G2_kbin;j++) ppt->xi_G2[i][j] = (ppt->xi_G2[i][j]+1.)/2.*_PI_;
    ...
    for (int index_thread = 0; index_thread<number_of_threads; index_thread++) {
      ...
      finufft_plan r2c_plan_G1;
      finufft_plan c2r_plan_G1_tau;
      ppt->list_r2c_plans_ncdmfinufft_G1[index_thread] = r2c_plan_G1;
      ppt->list_c2r_plans_ncdmfinufft_G1[index_thread] = c2r_plan_G1_tau;
	  ier = finufft_makeplan(3, dim, Ns1, 1, ppr->N_type, 1e-6, &ppt->list_r2c_plans_ncdmfinufft_G1[index_thread], &opts);
      ier = finufft_makeplan(3, dim, Ns3, -1, ppr->N_ell, 1e-6, &ppt->list_c2r_plans_ncdmfinufft_G1[index_thread], &opts);

      finufft_plan r2c_plan_G2;
      finufft_plan c2r_plan_G2_tau;
      ppt->list_r2c_plans_ncdmfinufft_G2[index_thread] = r2c_plan_G2;
      ppt->list_c2r_plans_ncdmfinufft_G2[index_thread] = c2r_plan_G2_tau;
      ier = finufft_makeplan(3, dim, Ns2, 1, ppr->N_type, 1e-6, &ppt->list_r2c_plans_ncdmfinufft_G2[index_thread], &opts);
      ier = finufft_makeplan(3, dim, Ns2_freq, -1, ppr->N_ell, 1e-6, &ppt->list_c2r_plans_ncdmfinufft_G2[index_thread], &opts);
...
int perturbations_workspace_init(
  ...
  // copy ncdm into workspace. It may be altered for ncdmfft.
  ppw->has_ncdm_psi_evolution = pba->has_ncdm;
  ...
  if (ppt->has_ncdmfft) {
    ...
    /** allocate xi_max_qbin to stroe xi_max fore each bin (NL) */
    int N_qbin = 0; 
    for (int i = 0; i < pba->N_ncdm; i++) N_qbin += pba->q_size_ncdm[i];
    
    class_alloc(ppw->xi_max_qbin, N_qbin * sizeof(double), ppt->error_message);
    class_alloc(ppw->xi_star_qbin, N_qbin * sizeof(double), ppt->error_message);

    /** - allocate FFT transformed spherical bessel functions */

    ppw->Acoeff = (Gcoefficients *) malloc(sizeof(Gcoefficients));
    ppw->Bcoeff = (Gcoefficients *) malloc(sizeof(Gcoefficients));

    class_alloc(ppw->fft_kernels_ncdmfft_G1, 2 * sizeof(double complex**), ppt->error_message);
    class_alloc(ppw->fft_kernels_ncdmfft_G1[0], ppr->N_ell * sizeof(double complex*), ppt->error_message);
    class_alloc(ppw->fft_kernels_ncdmfft_G1[1], ppr->N_ell * sizeof(double complex*), ppt->error_message);
    class_alloc(ppw->fft_kernels_ncdmfft_G2, 2 * sizeof(double complex**), ppt->error_message);
    class_alloc(ppw->fft_kernels_ncdmfft_G2[0], ppr->N_ell * sizeof(double complex*), ppt->error_message);
    class_alloc(ppw->fft_kernels_ncdmfft_G2[1], ppr->N_ell * sizeof(double complex*), ppt->error_message);

    for (int index_ell=0;index_ell<ppr->N_ell;index_ell++){
      ppw->fft_kernels_ncdmfft_G1[0][index_ell] = (double complex *) malloc(sizeof(double complex) * ppt->N_G1_freq_max);
      ppw->fft_kernels_ncdmfft_G1[1][index_ell] = (double complex *) malloc(sizeof(double complex) * ppt->N_G1_freq_max);
      ppw->fft_kernels_ncdmfft_G2[0][index_ell] = (double complex *) malloc(sizeof(double complex) * ppt->N_G2_freq_max);
      ppw->fft_kernels_ncdmfft_G2[1][index_ell] = (double complex *) malloc(sizeof(double complex) * ppt->N_G2_freq_max);
    }
  ...
  thread=omp_get_thread_num();
    ...
    ppw->r2c_plan_ncdmfinufft_G1 = ppt->list_r2c_plans_ncdmfinufft_G1[thread]; 
    ppw->c2r_plan_ncdmfinufft_G1 = ppt->list_c2r_plans_ncdmfinufft_G1[thread]; 
    ppw->r2c_plan_ncdmfinufft_G2 = ppt->list_r2c_plans_ncdmfinufft_G2[thread];
    ppw->c2r_plan_ncdmfinufft_G2 = ppt->list_c2r_plans_ncdmfinufft_G2[thread];
    
  }
  /** - k dependent ncdmfft quantities are allocated in the function 'perturbation_workspace_fill_ncdmfft'
   *  since array length varies for different k */
...
int perturbations_workspace_free (
  ...
  if (ppt->has_ncdmfft == _TRUE_) {
    free(ppw->fft_kernels_ncdmfft_G1);
    free(ppw->fft_kernels_ncdmfft_G2);

	finufft_destroy(ppw->r2c_plan_ncdmfinufft_G1); 
	finufft_destroy(ppw->c2r_plan_ncdmfinufft_G1); 
	finufft_destroy(ppw->r2c_plan_ncdmfinufft_G2); 
	finufft_destroy(ppw->c2r_plan_ncdmfinufft_G2); 
	free(ppw->xi_max_qbin);
	free(ppw->xi_star_qbin);
  }
...
int perturbations_solve(
  ...
  /* index for the ncdmfft loops */
  int index_loop_ncdmfft;
  /* - this pointer directs to the used tau sampling array. Either ppt->tau_sampling or ppt->tau_ncdmfft_sampling */
  double * working_tau_sampling;
  ...
  for (index_loop_ncdmfft=0; index_loop_ncdmfft<ppw->N_loops_ncdmfft; index_loop_ncdmfft++) {
    ...
    /* - if we have ncdmfft we need to modify the array of tau at which we want the quantities to be stored. 
      If we are either in the last loop or don't use ncdmfft the vanilla CLASS tau sampling is used.
    */
    if (ppw->N_loops_ncdmfft - index_loop_ncdmfft>1) {
      tau_actual_size = ppw->tau_ncdmfft_size;
      working_tau_sampling = ppw->tau_ncdmfft_sampling;
    }
    else {
      tau_actual_size = ppt->tau_size;
      working_tau_sampling = ppt->tau_sampling;
    }   
    ...
    class_call(perturbations_find_approximation_number(ppr,
                                                       ...
                                                       ppw,
                                                       tau,
                                                       working_tau_sampling[tau_actual_size-1],
                                                       &interval_number,
                                                       interval_number_of),
               ppt->error_message,
               ppt->error_message);
    ...
    for (index_interval=0; index_interval<interval_number; index_interval++) {
      ...
      // determine whether ncdm moments evolution is to be skipped. This is the case if we use ncdmfft and are NOT in fluid approximation or the first loop
      if ((index_loop_ncdmfft==0) || ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on ) {
        /* turn on ncdm_psi_evolution in first (or only) loop */
        ppw->has_ncdm_psi_evolution = _TRUE_; // we have psi_l evolution if we are in the first round or if we are in fluid approximation
      }
      else {
        /* turn off ncdm_psi_evolution for all other loops */
        ppw->has_ncdm_psi_evolution = _FALSE_; 
      }
    ...
    if (ppw->N_loops_ncdmfft - index_loop_ncdmfft>1) { 
      ...
      class_call(perturbations_calculate_psi_ncdmfft( ppr,
                                        pba,
                                        ppt,
                                        ppw,
                                        k),
    ...
    /** - free quantities allocated at the beginning of the loop */
    ...
    if (index_loop_ncdmfft == ppw->N_loops_ncdmfft-1){
      if (ppt->perturbations_verbose==2) printf("%d-th (final) evolution took total %f seconds %e mode\n\n", index_loop_ncdmfft, omp_get_wtime()-t_evolv, k);
...
int perturbations_vector_init(
        ...
        if (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_off){
          ...
          if ((ppt->has_ncdmfft) && (k*pba->conformal_age > ppr->l_max_ncdmfft)) {
            if (ppw->index_loop_ncdmfft!=0){
              ...
              ppv->l_max_ncdm[n_ncdm] = ppr->l_max_ncdmfft;
            }
          }
          else{
  ...
  if (_scalars_) {
    ...
    if (pba->has_ncdm == _TRUE_) {
      // [SG] no changes required since for ncdmfft_loop!=0: l_max_ncdm == -1
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if ((pa_old[ppw->index_ap_tca] == (int)tca_on) && (ppw->approx[ppw->index_ap_tca] == (int)tca_off)) {
        ...
        if (ppw->has_ncdm_psi_evolution == _TRUE_) {
          index_pt = 0;
          for (n_ncdm = 0; n_ncdm < ppv->N_ncdm; n_ncdm++){
            for (index_q=0; index_q < ppv->q_size_ncdm[n_ncdm]; index_q++){
              for (l=0; l<=ppv->l_max_ncdm[n_ncdm];l++){
...
int perturbations_total_stress_energy(
  ...
  double tau;
  background_tau_of_z (pba,1./a-1,&tau);
  ...
  if (_scalars_) {
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      if (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on){
        ...
        double rho_plus_p_shear_ncdmfft;
        int last_index;
        ErrorMsg em = "Interpolation psi error";

        if(tau>ppw->tau_ncdmfft_sampling[ppw->tau_ncdmfft_size-1])
        {
          tau_eval = ppw->tau_ncdmfft_sampling[ppw->tau_ncdmfft_size-1];
        }
        else
        {
          tau_eval = tau;
        ...
        for (n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
          ...
          if (ppw->has_ncdm_psi_evolution == _TRUE_) {
            ...
            double rho_delta_ncdmfft, rho_plus_p_theta_ncdmfft, rho_plus_p_shear_ncdmfft, delta_p_ncdmfft;
            int last_index;
            ErrorMsg em = "Interpolation psi error";
            if (tau>ppw->tau_ncdmfft_sampling[ppw->tau_ncdmfft_size-1])
            {
            ...
            else if (tau<ppw->tau_ncdmfft_sampling[0])
            {
              tau_eval = ppw->tau_ncdmfft_sampling[0];
            }
            else
            {
              tau_eval = tau;
            }
            // interpolate the perturbations at tau
            class_call(array_interpolate_spline(
                ppw->tau_ncdmfft_sampling,
                ppw->tau_ncdmfft_size,
                ppw->rho_delta_ncdm_ncdmfft[n_ncdm],
                ppw->rho_delta_ncdm_ncdmfft_spline[n_ncdm],
                1,
                tau_eval,
                &last_index,
                &rho_delta_ncdmfft,
            ...
            class_call(array_interpolate_spline(
                ppw->tau_ncdmfft_sampling,
                ppw->tau_ncdmfft_size,
                ppw->rho_plus_p_theta_ncdm_ncdmfft[n_ncdm],
                ppw->rho_plus_p_theta_ncdm_ncdmfft_spline[n_ncdm],
                1,
                tau_eval,
                &last_index,
                &rho_plus_p_theta_ncdmfft,
                1,
                em
              ),
              ppt->error_message,
            ...
            class_call(array_interpolate_spline(
                ppw->tau_ncdmfft_sampling,
                ppw->tau_ncdmfft_size,
                ppw->rho_plus_p_shear_ncdm_ncdmfft[n_ncdm],
                ppw->rho_plus_p_shear_ncdm_ncdmfft_spline[n_ncdm],
                1,
                tau_eval,
                &last_index,
                &rho_plus_p_shear_ncdmfft,
                1,
                em
              ),
              ppt->error_message,
            ...
            class_call(array_interpolate_spline(
                ppw->tau_ncdmfft_sampling,
                ppw->tau_ncdmfft_size,
                ppw->delta_p_ncdm_ncdmfft[n_ncdm],
                ppw->delta_p_ncdm_ncdmfft_spline[n_ncdm],
                1,
                tau_eval,
                &last_index,
                &delta_p_ncdmfft,
              ...
              rho_delta_ncdm = rho_delta_ncdmfft;
              rho_plus_p_theta_ncdm = rho_plus_p_theta_ncdmfft;
              rho_plus_p_shear_ncdm = rho_plus_p_shear_ncdmfft;
              delta_p_ncdm = delta_p_ncdmfft;
...
int perturbations_sources(
  ...
  if (_scalars_) {
    ...
    if (ppw->N_loops_ncdmfft - ppw->index_loop_ncdmfft > 1)  {
      ...
      int index_nq = 0;
      /** Store the early-time results from Boltzmann Hierarchy (correct up to tau ~ lmax/k) */
      if (ppw->index_loop_ncdmfft==0) {
...
/**
 * Compute the psi_l with the ncdmfft method descibed in 2201.11129.
 * 
 *
 * @param ppr                      Input: pointer to precision structure
 * @param pba                      Input: pointer to background structure
 * @param ppt                      Input: pointer to perturbation structure
 * @param ppw                      Input/Output: in input, fixed parameters (e.g. indices); in output, psi_l
 * @param k                        Input: wavenumber
 */

int perturbations_calculate_psi_ncdmfft(struct precision * ppr,
                          struct background * pba,
                          struct perturbations * ppt,
                          struct perturbations_workspace * ppw,
                          double k)

{
  ...
  for (int n_ncdm = pba->N_ncdm-1; n_ncdm > -1; n_ncdm --) {
    ...
    for (int index_q = pba->q_size_ncdm[n_ncdm]-1; index_q > -1; index_q--) {
      ...
      psi_0 = ppw->psi_0_ncdmfft[index_nq];
      psi_1 = ppw->psi_1_ncdmfft[index_nq];
      psi_2 = ppw->psi_2_ncdmfft[index_nq];
     
      // initialize by the initial-condition term
    ...
    // compute the ncdm perturbation quantities
    for (int index_tau=0; index_tau < ppw->tau_ncdmfft_size; index_tau++) {
      // initialize the perturbation to zero
      ppw->rho_delta_ncdm_ncdmfft[n_ncdm][index_tau] = 0.;
      ppw->rho_delta_ncdm_ncdmfft[n_ncdm][index_tau+ppw->tau_ncdmfft_size] = 0.;
      ppw->rho_delta_ncdm_ncdmfft_spline[n_ncdm][index_tau] = 0.;
      ppw->rho_plus_p_theta_ncdm_ncdmfft[n_ncdm][index_tau] = 0.;
      ppw->rho_plus_p_theta_ncdm_ncdmfft[n_ncdm][index_tau+ppw->tau_ncdmfft_size] = 0.;    
      ppw->rho_plus_p_theta_ncdm_ncdmfft_spline[n_ncdm][index_tau] = 0.;
      ppw->rho_plus_p_shear_ncdm_ncdmfft[n_ncdm][index_tau] = 0.;
      ppw->rho_plus_p_shear_ncdm_ncdmfft[n_ncdm][index_tau+ppw->tau_ncdmfft_size] = 0.;
      ppw->rho_plus_p_shear_ncdm_ncdmfft_spline[n_ncdm][index_tau] = 0.;
      ppw->delta_p_ncdm_ncdmfft[n_ncdm][index_tau] = 0.;
      ppw->delta_p_ncdm_ncdmfft[n_ncdm][index_tau+ppw->tau_ncdmfft_size] = 0.;
      ppw->delta_p_ncdm_ncdmfft_spline[n_ncdm][index_tau] = 0.;

      a2 = ppw->a_ncdmfft[index_tau]*ppw->a_ncdmfft[index_tau];
   
      if (n_ncdm==0) index_nq2 = 0;
      else{
	    index_nq2 = 0;
        for (int i=0;i<n_ncdm;i++) index_nq2 += pba->q_size_ncdm[i];
      }
      // looping over ncdm momenta for that species
      for (int index_q = 0; index_q < pba->q_size_ncdm[n_ncdm]; index_q++) {
        q = pba->q_ncdm[n_ncdm][index_q];
        q2 = q*q;
        epsilon = sqrt(q2+pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm]*a2);
        ppw->rho_delta_ncdm_ncdmfft[n_ncdm][index_tau] += q2*epsilon*pba->w_ncdm[n_ncdm][index_q]*ppw->psi_0_ncdmfft[index_nq2][index_tau];
        ppw->rho_plus_p_theta_ncdm_ncdmfft[n_ncdm][index_tau] += q2*q*pba->w_ncdm[n_ncdm][index_q]*ppw->psi_1_ncdmfft[index_nq2][index_tau];
        ppw->rho_plus_p_shear_ncdm_ncdmfft[n_ncdm][index_tau] += q2*q2/epsilon*pba->w_ncdm[n_ncdm][index_q]*ppw->psi_2_ncdmfft[index_nq2][index_tau];
        ppw->delta_p_ncdm_ncdmfft[n_ncdm][index_tau] += q2*q2/epsilon*pba->w_ncdm[n_ncdm][index_q]*ppw->psi_0_ncdmfft[index_nq2][index_tau];
    ...
    // calculate the splines for the ncdm perturbation quantities
    class_call(
      array_spline_table_lines(ppw->tau_ncdmfft_sampling,
                          ppw->tau_ncdmfft_size,
                          ppw->rho_delta_ncdm_ncdmfft[n_ncdm],
                          1,
                          ppw->rho_delta_ncdm_ncdmfft_spline[n_ncdm],
    ...
    class_call(
      array_derive_spline(ppw->tau_ncdmfft_sampling,
                          ppw->tau_ncdmfft_size,
                          ppw->rho_delta_ncdm_ncdmfft[n_ncdm],
                          ppw->rho_delta_ncdm_ncdmfft_spline[n_ncdm],
                          1,
...
int perturbations_initial_condition_ncdmfft(
  struct background * pba,
  struct perturbations * ppt,
  struct perturbations_workspace * ppw,
  double k, 
...

{
  ...
  for (int index_tau = 0; index_tau < ppw->tau_ncdmfft_size; index_tau++) {
    ...
    auxiliary_function_ncdmfft(ell, 1, x[index_tau], &temp); ic_term += - (2.0*1.0+1.0) * temp * psi_1_ic;
    auxiliary_function_ncdmfft(ell, 2, x[index_tau], &temp); ic_term += + (2.0*2.0+1.0) * temp * psi_2_ic;
    accumulator[index_tau] = ic_term;
  }
...

{
  ...
  int ell;
  double *accumulator;
  double * GA_of_tau, * GB_of_tau;
  double *G_of_x1, *G_of_x2;
  double *G0_of_x1, *G0_of_x2;
  int last_index;
  double q, a, e;
  ErrorMsg em;
  ...
  freq_G2 = (double *) malloc(sizeof(double) * N_G2_freq);
  w8_freq_G2 = (double *) malloc(sizeof(double) * N_G2_freq);
  for (int i=0;i<(N_G2_freq-1)/2+1;i++) {
    freq_G2[(N_G2_freq-1)/2+i] = i/10.;
    freq_G2[(N_G2_freq-1)/2-i] = -1.*i/10.;
    w8_freq_G2[(N_G2_freq-1)/2+i] = 1./10.;
    w8_freq_G2[(N_G2_freq-1)/2-i] = 1./10.;
  }

  class_alloc(GA_of_tau, ppw->tau_ncdmfft_size*sizeof(double), ppt->error_message);
  class_alloc(GB_of_tau, ppw->tau_ncdmfft_size*sizeof(double), ppt->error_message);
  //initialize to zero
  for (int index_tau = 0; index_tau < ppw->tau_ncdmfft_size; index_tau ++) {
    GA_of_tau[index_tau] = 0.0;
    GB_of_tau[index_tau] = 0.0;
  }
  ...
  for (int index_tau = 0; index_tau < ppw->tau_ncdmfft_size; index_tau ++) {
    ...
    GA_of_tau[index_tau] = (e/q/k) * pba->dlnf0_dlnq_ncdm[n_ncdm][index_q];
    GB_of_tau[index_tau] = (e/q/k) * pba->dlnf0_dlnq_ncdm[n_ncdm][index_q];
    GA_of_tau[index_tau] *= - ppw->eta_prime_ncdmfft[index_tau];
    GB_of_tau[index_tau] *= - 0.5 * ppw->h_prime_ncdmfft[index_tau] - 3.0 * ppw->eta_prime_ncdmfft[index_tau];
  }
  ...
  double G_i, Gp_i, G_i_p, G_m, Gp_m, G_m_m, G_m_p, G_f, Gp_f, G_f_m;
  double Dlogxi = 0.001;
  ...
  G0_coeff (ppw->Acoeff, xi_i, xi_m, xi_f, G_i, Gp_i, G_m, Gp_m, G_f, Gp_f);
  A_a1 = ppw->Acoeff->a1, A_b1 = ppw->Acoeff->b1, A_c1 = ppw->Acoeff->c1, A_d1 = ppw->Acoeff->d1;
  ...
  G0_coeff (ppw->Bcoeff, xi_i, xi_m, xi_f, G_i, Gp_i, G_m, Gp_m, G_f, Gp_f);
  B_a1 = ppw->Bcoeff->a1, B_b1 = ppw->Bcoeff->b1, B_c1 = ppw->Bcoeff->c1, B_d1 = ppw->Bcoeff->d1;
  ...
  double complex *G1, *G2, *finufft_G2;
  double complex *KG1, *KG2;
  double *KG0, *KG_total;
  KG_total = (double *) malloc(sizeof(double) * ( (int) (ppw->tau_ncdmfft_size*ppr->N_ell)));
  xi_G2_out = (double *) malloc(sizeof(double) * ppw->tau_ncdmfft_size);
  xi_G1_out = (double *) malloc(sizeof(double) * ppw->tau_ncdmfft_size);
  KG1 = (double complex *) malloc(sizeof(double complex) * (ppw->tau_ncdmfft_size*ppr->N_ell));
  KG2 = (double complex *) malloc(sizeof(double complex) * (ppw->tau_ncdmfft_size*ppr->N_ell));
  G1 = (double complex *) malloc(sizeof(double complex) * N_G1*ppr->N_type);
  G2 = (double complex *) malloc(sizeof(double complex) * N_G2*ppr->N_type);
  KG0 = (double *) malloc(sizeof(double) * (ppw->tau_ncdmfft_size*ppr->N_ell));
  finufft_G2 = (double complex *) malloc(sizeof(double complex) * N_G2_freq*ppr->N_type);
  double *xi_G1_logscale;
  xi_G1_logscale = (double *) malloc(sizeof(double) * N_G1);
  ...
  for (int i=0;i<N_G1;i++){
    // ppt->xi_G1 is N_G1-th ordr GL, which forms a set of log(1+xi) points
    xx = (1+ppt->xi_G1[i])*log(1+xi_m)/2;
    xx = exp(xx)-1;         
    ...
    G1[i] = (double complex) (GG - (A_a1 + A_b1*xx + A_c1*xx*xx + A_d1*xx*xx*xx));
	...
	G1[i+N_G1] = G1[i+N_G1]*ppt->w8_G1[i]*log(1+xi_m)/2.*(1+xx);
  }
  ...
  if ((ppr->ncdmfft_bigq_approximation==0)||(xi_m != ppr->xi_m)||((ppw->index_loop_ncdmfft<ppw->N_loops_ncdmfft-1)&&(ppr->ncdmfft_bigq_approximation==1)&& ((index_q==index_maxq)||(fabs(ppr->xi_m/k-tau_at_xi_m)/tau_at_xi_m>0.001)))){
    ...
    freq_G1 = (double *) malloc(sizeof(double) * N_freq1);
    w8_freq_G1 = (double *) malloc(sizeof(double) * N_freq1);
    finufft_G1 = (double complex *) malloc(sizeof(double complex) * N_freq1*ppr->N_type);
     
    for (int i=0;i<(N_freq1-1)/2+1;i++) {
      freq_G1[(N_freq1-1)/2+i] = i* ((double) N_G1) / ((double) N_freq1);
      freq_G1[(N_freq1-1)/2-i] = -1*i* ((double) N_G1) / ((double) N_freq1);
      w8_freq_G1[(N_freq1-1)/2+i] = ((double) N_G1) / ((double) N_freq1);
      w8_freq_G1[(N_freq1-1)/2-i] = ((double) N_G1) / ((double) N_freq1);
    }
    ...
    for (ell=0;ell<ppr->N_ell;ell++){
      for (int index_type=0;index_type<ppr->N_type;index_type++){
        if (index_type==0){
          for (int i=0;i<N_freq1;i++) integrand_G1[ell*N_freq1+i] = finufft_G1[index_type*N_freq1+i] * ppw->fft_kernels_ncdmfft_G1[index_type][ell][i]/(2*_PI_)*(_PI_/xi_m)* w8_freq_G1[i];
        }
    ...
    for (ell=0;ell<ppr->N_ell;ell++){
      for (int i=0;i<ppw->tau_ncdmfft_size;i++) 
        ppw->KG1_maxq[ell*ppw->tau_ncdmfft_size+i] = creal(KG1[ell*ppw->tau_ncdmfft_size+i])/pba->dlnf0_dlnq_ncdm[n_ncdm][index_q];
      
    }
	free(integrand_G1);
    free(w8_freq_G1);
	free(freq_G1);free(finufft_G1);
  }
  else{
    double temp_KG1;
    for (ell=0;ell<ppr->N_ell;ell++){
  ...
  for (int i=0;i<N_G2;i++){
    ...
    if (xx>=xi_m){
      ...
      G2[i+N_G2] = G2[i+N_G2]*ppw->w8_G2[i]*xi_f/2;
    }
    else{
      G2[i] = (double complex) 0;
      G2[i+N_G2] = (double complex) 0;
    }
  }
  ...
  for (ell=0;ell<ppr->N_ell;ell++){
    for (int index_type=0;index_type<ppr->N_type;index_type++){
	  if (index_type == 0){
      for (int i=0;i<N_G2_freq;i++) integrand_G2[ell*N_G2_freq+i] = finufft_G2[index_type*N_G2_freq+i] * ppw->fft_kernels_ncdmfft_G2[index_type][ell][i]/(2*_PI_)*(_PI_/xi_f) * w8_freq_G2[i];
	  }
	  else {
      for (int i=0;i<N_G2_freq;i++) integrand_G2[ell*N_G2_freq+i] += finufft_G2[index_type*N_G2_freq+i] * ppw->fft_kernels_ncdmfft_G2[index_type][ell][i]/(2*_PI_)*(_PI_/xi_f) * w8_freq_G2[i];
	  }
  ...
  for (ell=0;ell<ppr->N_ell;ell++){
    ...
    if (ell==0) accumulator = accumulator_0;
  ...
  free(xi_G1_logscale);
  free(freq_G2); free(w8_freq_G2);
  free(G1);free(G2);
  free(xi_G2_out);free(xi_G1_out);free(KG2);free(KG1);
  free(finufft_G2);
  free(GA_of_tau);
  free(GB_of_tau);
  free(integrand_G2);
...
/**
 * @brief this function is called everytime a new k is loaded into workspace. It will calculate all quantities for the ncdmfft stuff
 * 
 * @param ppr 
 * @param pba 
 * @param ppt 
...
int perturbation_workspace_fill_ncdmfft(struct precision * ppr,
                                struct background * pba,
                                struct perturbations * ppt,
                                struct perturbations_workspace * ppw,
                                double k,
                                double tau_ini)
{
  ...
  double *chi_ini;
  chi_ini = (double *) malloc(sizeof(double) * N_qbin);
  index_nq = 0;
  for (int index_n=0;index_n<pba->N_ncdm;index_n++){
    for (int i=0;i<pba->q_size_ncdm[index_n];i++) {
		chi_ini[index_nq] = pvecback[pba->index_bg_chi_ncdmfft + index_nq]; // (NL) Need to modify for the case of N_ncdm > 1
        index_nq += 1;
	}
  }

  tau_xi_max = pba->conformal_age;

  // call and interpolate background values
  class_call(background_at_tau(pba,
                                tau_xi_max,
                                long_info,
                                inter_normal,
  ...
  for (int i = 0; i < pba->N_ncdm; i++) {
    for (int j = 0; j < pba->q_size_ncdm[i]; j++) {
      if (pvecback[pba->index_bg_chi_ncdmfft+i*pba->q_size_ncdm[i]+j]*k>xi_max) {
		...
		ppw->xi_star_qbin[index_nq] = fmin(ppr->xi_m, xi_max/2.); 
  ...
  // I found that the sampling size is almost proportional to k
  ppw->tau_ncdmfft_size = MAX(ppr->N_tau_ncdmfft_size_min,ppr->N_tau_ncdmfft_size_scale*pow(k,0.5));
  int idx;
  if (k<=0.1) idx = 0;
  else if (k<=1) idx = 1;
  else if (k<=5) idx = 2;
  ...
  if (idx==0) ppw->N_G2 = 1000;
  else if (idx==1) ppw->N_G2 = 5000;
  ...
  if (ppw->tau_ncdmfft_size==1){
    ppw->tau_ncdmfft_size = 2;
  }

  /** Allocate the arrays that will be used by the solver and the fft approach to communicate. 
   * This depends in fact on the chosen k */
  ...
  /** allocate KG1 (convolution with G1 for big-q approx) */
  class_alloc(ppw->KG1_maxq,ppr->N_ell*ppw->tau_ncdmfft_size * sizeof(double),ppt->error_message);

  /** allocate the ncdm perturation quantities */
  class_alloc(ppw->rho_delta_ncdm_ncdmfft,pba->N_ncdm * sizeof(double *), ppt->error_message); // factor of 2 due to spline function
  class_alloc(ppw->rho_delta_ncdm_ncdmfft_spline, pba->N_ncdm * sizeof(double *), ppt->error_message);
  class_alloc(ppw->rho_plus_p_theta_ncdm_ncdmfft, pba->N_ncdm * sizeof(double *), ppt->error_message);
  class_alloc(ppw->rho_plus_p_theta_ncdm_ncdmfft_spline, pba->N_ncdm * sizeof(double *), ppt->error_message);
  class_alloc(ppw->rho_plus_p_shear_ncdm_ncdmfft, pba->N_ncdm * sizeof(double *), ppt->error_message);
  class_alloc(ppw->rho_plus_p_shear_ncdm_ncdmfft_spline, pba->N_ncdm * sizeof(double *), ppt->error_message);
  class_alloc(ppw->delta_p_ncdm_ncdmfft, pba->N_ncdm * sizeof(double *), ppt->error_message);
  class_alloc(ppw->delta_p_ncdm_ncdmfft_spline, pba->N_ncdm * sizeof(double *), ppt->error_message);
  
  index_nq = 0;
  for (int index_n=0;index_n<pba->N_ncdm;index_n++) {
    for (int index_q=0; index_q<pba->q_size_ncdm[index_n]; index_q++) {
      class_alloc(ppw->psi_0_ncdmfft[index_nq],ppw->tau_ncdmfft_size * sizeof(double),ppt->error_message);
    ...
    /** allocate the ncdm perturation quantities */
    class_alloc(ppw->rho_delta_ncdm_ncdmfft[index_n],2 * ppw->tau_ncdmfft_size * sizeof(double), ppt->error_message); // factor of 2 due to spline function
    class_alloc(ppw->rho_delta_ncdm_ncdmfft_spline[index_n], ppw->tau_ncdmfft_size * sizeof(double), ppt->error_message);
    class_alloc(ppw->rho_plus_p_theta_ncdm_ncdmfft[index_n], 2 * ppw->tau_ncdmfft_size * sizeof(double), ppt->error_message);
    class_alloc(ppw->rho_plus_p_theta_ncdm_ncdmfft_spline[index_n], ppw->tau_ncdmfft_size * sizeof(double), ppt->error_message);
    class_alloc(ppw->rho_plus_p_shear_ncdm_ncdmfft[index_n], 2 * ppw->tau_ncdmfft_size * sizeof(double), ppt->error_message);
    class_alloc(ppw->rho_plus_p_shear_ncdm_ncdmfft_spline[index_n], ppw->tau_ncdmfft_size * sizeof(double), ppt->error_message);
    class_alloc(ppw->delta_p_ncdm_ncdmfft[index_n], 2 * ppw->tau_ncdmfft_size * sizeof(double), ppt->error_message);
    class_alloc(ppw->delta_p_ncdm_ncdmfft_spline[index_n], ppw->tau_ncdmfft_size * sizeof(double), ppt->error_message);

  }

  /** allocate tau_ncdmfft */
  class_alloc(ppw->tau_ncdmfft_sampling, ppw->tau_ncdmfft_size * sizeof(double), ppt->error_message);

  /** allocate a_ncdmfft */
  class_alloc(ppw->a_ncdmfft, ppw->tau_ncdmfft_size * sizeof(double), ppt->error_message);

  /**  Sample the tau_ncdmfft_sampling array */
  for (int index_tau=0; index_tau<ppw->tau_ncdmfft_size; index_tau++) {
	
  	 if (index_tau <= (int) (ppw->tau_ncdmfft_size/2)){
        tau = log(tau_ini) + log(1000/tau_ini)/( (int) (ppw->tau_ncdmfft_size/2) )*index_tau;
        tau = exp(tau);
  	 }
  	  else{
		  tau = tau_ini + 1000 + (tau_xi_max - tau_ini - 1000)/( ppw->tau_ncdmfft_size - (int) (ppw->tau_ncdmfft_size/2) ) * (index_tau - (int) (ppw->tau_ncdmfft_size/2));
	  }

	if (tau >= tau_xi_max) tau = tau_xi_max;
	if (index_tau == 0) tau = tau_ini;
	if (index_tau == ppw->tau_ncdmfft_size-1) tau = tau_xi_max;

    // TODO: can we sample in a better way?
    ppw->tau_ncdmfft_sampling[index_tau] = tau;
    // call and interpolate background values
    class_call(background_at_tau(pba,
                                  tau,
                                  long_info,
    ...
    ppw->a_ncdmfft[index_tau] = pvecback[pba->index_bg_a];
    ...
    for (int index_n=0; index_n<pba->N_ncdm; index_n++) {
      for (int index_q=0; index_q<pba->q_size_ncdm[index_n]; index_q++) {
        if(index_tau==0){
          chi_offset[index_nq] =
          pvecback[pba->index_bg_chi_ncdmfft + index_nq];
        }
...
fclose(fout);
  ...
  // The coding number of loops is 1+the logical number of loops. The first loop which calculated the initial conditions is not counted by the precision parameter.
  ppw->N_loops_ncdmfft = ppr->N_loops_ncdmfft+1;
  //if (k<=1) ppw->N_loops_ncdmfft = 2;
...
/** 
 * deallocates the ncdmfft objects from the workspace
 * */
int perturbation_workspace_free_ncdmfft(struct perturbations * ppt,
                                        struct background * pba,
                                        struct perturbations_workspace * ppw
                                        )
{
  ...
  for (int intex_n=0;intex_n<pba->N_ncdm;intex_n++){
    ...
    free(ppw->rho_delta_ncdm_ncdmfft_spline[intex_n]);
    free(ppw->rho_plus_p_theta_ncdm_ncdmfft[intex_n]);
    free(ppw->rho_plus_p_theta_ncdm_ncdmfft_spline[intex_n]);
    free(ppw->rho_plus_p_shear_ncdm_ncdmfft[intex_n]);
    free(ppw->rho_plus_p_shear_ncdm_ncdmfft_spline[intex_n]);
    free(ppw->delta_p_ncdm_ncdmfft[intex_n]);
    free(ppw->delta_p_ncdm_ncdmfft_spline[intex_n]);
  }
