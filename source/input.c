...
int input_read_parameters_general(struct file_content * pfc,
  ...
  if (ppt->has_perturbations == _TRUE_) {
    ...
    /** 2a) Perturbed recombination */
    /* Read */
    class_read_flag_or_deprecated("perturbed_recombination","perturbed recombination",ppt->has_perturbed_recombination);

    /** 2b) FFT iterative method of 2201.11129 for ncdm perturbations */
    class_read_flag("ncdmfft",ppt->has_ncdmfft);
    class_read_flag("ncdmfft",pba->has_ncdmfft); //we need it in the background for the chi integration
