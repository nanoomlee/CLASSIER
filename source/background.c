  ...
  if (pba->has_ncdmfft == _TRUE_) {
    ...
    for(int i=0; i<pba->N_ncdm; i++) {
      ...
      for(int j=0; j<pba->q_size_ncdm[i]; j++) {
        //pvecback[pba->index_bg_chi_ncdmfft + i*pba->q_size_ncdm[i] + j] = pvecback_B[pba->index_bi_chi_ncdmfft + i*pba->q_size_ncdm[i] + j];
        pvecback[pba->index_bg_chi_ncdmfft + index_nq] = pvecback_B[pba->index_bi_chi_ncdmfft + index_nq];
		index_nq += 1;
      }
    }
  }
...
int background_init(
  ...
  if (pba->background_verbose > 0) {
    printf("Running CLASSIER (CLASS version %s modified with Integral-Equation approach for non-cold relics)\n",_VERSION_);
...
int background_indices(
  ...
  if (pba->Omega0_ncdm_tot != 0.)
    ...
    pba->has_ncdmfft = _FALSE_; //we wont need it otherwise ...
  ...
  int ncdm_counter = 0; 
  for (int i = 0; i < pba->N_ncdm; i++){
    ncdm_counter += pba->q_size_ncdm[i];
  }
  class_define_index(pba->index_bg_chi_ncdmfft,pba->has_ncdmfft,index_bg,ncdm_counter);
  ...
  if (pba->has_ncdmfft == _TRUE_){
    ...
    class_define_index(pba->index_bi_chi_ncdmfft,_TRUE_,index_bi,ncdm_counter);
  }
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    if (pba->has_ncdmfft == _TRUE_) {
      ...
      for (int i=0; i<pba->N_ncdm; i++) {
        for (int j=0; j<pba->q_size_ncdm[i]; j++) {
          class_store_double(dataptr,pvecback[pba->index_bg_chi_ncdmfft+index_nq],_TRUE_,storeidx);
          index_nq += 1;
        }
      }
    }
