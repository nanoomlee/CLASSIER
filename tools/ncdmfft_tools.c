...
int G0_coeff2 (Gcoefficients2 * coeff, double xi_i, double xi_m, double G_i, double Gp_i, double G_m, double Gp_m) {
    ...
    double a = G_i - dGdx*xi_i - beta * xbeta *xi_i*xi_m;
    double b = dGdx  + beta *(xbeta*xi_i + xi_i*xi_m + xi_m*xbeta);
    double c = -beta * (xi_m + xi_i + xbeta);
    double d = beta;
	coeff->a = a; coeff->b = b; coeff->c = c; coeff->d = d;
...
int compute_I0_vectorized_psi0only(       // returns the six I0s for a given xi
    double *xi_list,         //   xi
    int Nxi,
    double xi1,         //   xi1
    double xi0,             //   xi0
	...
	if (xi0<=-100.) {
		xi1 = xi0;
		coeff->a1 = coeff->a2; coeff->b1 = coeff->b2; coeff->c1 = coeff->c2; coeff->d1 = coeff->d2;
	}

    // calI[l][n][v] holds integrals for l=0,1,2,3,4,5 (with 3, 4, 5
    // the second derivatives); n the powers of xi; v the values at 0, xi-xi1,xi

    //calI = (double **)malloc(4 * sizeof(double *));
    //for (n = 0; n < 4; n++) {
    //    calI[n] = (double *)malloc(3 * sizeof(double));
    //}
    calI = (double ***)malloc(1 * sizeof(double **));
    for (l = 0; l < 1; l++) {
      calI[l] = (double **)malloc(4 * sizeof(double *));
      for (n = 0; n < 4; n++) {
        calI[l][n] = (double *)malloc(3 * sizeof(double));
      }
    }
    ...
    if (type=='A'){
        if (l==0){
        calI[l][0][0] = 0.0;  calI[l][1][0] = -1.0;
        calI[l][2][0] = 0.0;  calI[l][3][0] = (2.0);
        }
        else if (l==1){
        //  l=1
        calI[l][0][0] = -1.0;  calI[l][1][0] = 0.0;
        calI[l][2][0] = -2.0;  calI[l][3][0] = 0.0;
        }
        else if (l==2){
        //  l=2
        calI[l][0][0] = 0.0;  calI[l][1][0] = -2.0;
        calI[l][2][0] = 0.0;  calI[l][3][0] = (-8.0);
        }
    }
    else if (type=='B'){
        if (l==0){
            //  l=3    this is actually j_0''
            calI[l][0][0] = 0.0;  calI[l][1][0] = -1.0;
            //calI[2][0] = 0.0;  calI[3][0] = -10.0;
            calI[l][2][0] = 0.0;  calI[l][3][0] = -6.0;
        }
        else if (l==1){
            //  l=4    this is actually j_1''
            calI[l][0][0] = 1.0/3.0;  calI[l][1][0] = 0.0;
            calI[l][2][0] = -2.0; calI[l][3][0] = 0.0;
        }
        else if (l==2){
            //  l=5    this is actually j_2''
            calI[l][0][0] = 0.0;
            calI[l][1][0] = 0.0;
            calI[l][2][0] =0.0;
            calI[l][3][0] = -12.0;
    ...
    for (int i=0;i<Nxi;i++){
		...
		if (type=='A'){
            if (l==0){
                //  l=0
                calI[l][0][2] = Si0;  calI[l][1][2] = -c0;  
                calI[l][2][2] = -xi*c0+s0;  calI[l][3][2] = (2.0-xi*xi)*c0+2.0*xi*s0;
            }
            else if (l==1){
                //  l=1
                calI[l][0][2] = -s0/xi;  calI[l][1][2] = Si0-s0;
                calI[l][2][2] = -2.0*c0-xi*s0;  calI[l][3][2] = -3.0*xi*c0+(3.0-xi*xi)*s0;
            }
            else if (l==2){
                //  l=2
                calI[l][0][2] = (1.5/xi)*(c0-s0/xi)+Si0/2.0;  calI[l][1][2] = c0-3.0*s0/xi;
                calI[l][2][2] = xi*c0-4.0*s0+3.0*Si0;  calI[l][3][2] = (xi*xi-8.0)*c0-5.0*xi*s0;
            }
        }
        else if (type=='B'){
            if (l==0){
                //  l=3    this is actually j_0''
                calI[l][0][2] = c0/xi-s0/sqr(xi);  calI[l][1][2] = c0-2.0*s0/xi;
                calI[l][2][2] = xi*c0-3.0*s0+2.0*Si0;  calI[l][3][2] = (xi*xi-6.0)*c0-4.0*xi*s0;
            }
            else if (l==1){
                //  l=4    this is actually j_1''
                calI[l][0][2] = 2.0*c0/sqr(xi)+s0/xi*(1.0-2.0/sqr(xi));  calI[l][1][2] = 3.0*c0/xi+(xi*xi-3.0)*s0/sqr(xi);
                calI[l][2][2] =4.0*c0+(xi-6.0/xi)*s0;  calI[l][3][2] = 5.0*xi*c0-(11.0-xi*xi)*s0+6.0*Si0;
            }
            else if (l==2){
                //  l=5    this is actually j_2''
                calI[l][0][2] = (-(xi*(-9.0 + xi*xi)*c0) + (-9.0 + 4.0*xi*xi)*s0)/pow(xi,4.0);  
                calI[l][1][2] = ((12.0-xi*xi)*xi*c0+(5.0*xi*xi-12.0)*s0)/pow(xi,3.0);
                calI[l][2][2] = (18.0-xi*xi)*c0/xi-(6.0*(3.0-xi*xi)*s0)/sqr(xi)+Si0;
                calI[l][3][2] = (24.0-xi*xi)*c0+(7.0*xi*xi-36.0)*s0/xi;
            }
        }
		}
        ...
        if (xi > xi1) {
            ...
            if (type=='A'){
                if (l==0){
                    ...
                    calI[l][0][1] = Si1;
                    calI[l][1][1] = -c1;
                    calI[l][2][1] = -xid * c1 + s1;
                    calI[l][3][1] = (2.0 - xid * xid) * c1 + 2.0 * xid * s1;
                }
                else if (l==1){
                    //  l=1
                    calI[l][0][1] = -s1 / xid;
                    calI[l][1][1] = Si1 - s1;
                    calI[l][2][1] = -2.0 * c1 - xid * s1;
                    calI[l][3][1] = -3.0 * xid * c1 + (3.0 - xid * xid) * s1;
                }
                else if (l==2){
                    //  l=2
                    calI[l][0][1] = (1.5 / xid) * (c1 - s1 / xid) + Si1 / 2.0;
                    calI[l][1][1] = c1 - 3.0 * s1 / xid;
                    calI[l][2][1] = xid * c1 - 4.0 * s1 + 3.0 * Si1;
                    calI[l][3][1] = (xid * xid - 8.0) * c1 - 5.0 * xid * s1;
                }
            }
            else if (type=='B'){
                if (l==0){
                    //  l=3    this is actually j_0''
                    calI[l][0][1] = c1 / xid - s1 / sqr(xid);
                    calI[l][1][1] = c1 - 2.0 * s1 / xid;
                    calI[l][2][1] = xid * c1 - 3.0 * s1 + 2.0 * Si1;
                    calI[l][3][1] = (xid * xid - 6.0) * c1 - 4.0 * xid * s1;
                }
                else if (l==1){
                    //  l=4    this is actually j_1''
                    calI[l][0][1] = 2.0 * c1 / sqr(xid) + s1 / xid * (1.0 - 2.0 / sqr(xid));
                    calI[l][1][1] = 3.0 * c1 / xid + (xid * xid - 3.0) * s1 / sqr(xid);
                    calI[l][2][1] = 4.0 * c1 + (xid - 6.0 / xid) * s1;
                    calI[l][3][1] = 5.0 * xid * c1 - (11.0 - xid * xid) * s1 + 6.0 * Si1;
                }
                else if (l==2){
                    //  l=5    this is actually j_2''
                    calI[l][0][1] = (-(xid * (-9.0 + xid * xid) * c1) + (-9.0 + 4.0 * xid * xid) * s1) / pow(xid, 4.0);
                    calI[l][1][1] = ((12.0 - xid * xid) * xid * c1 + (5.0 * xid * xid - 12.0) * s1) / pow(xid, 3.0);
                    calI[l][2][1] = (18.0 - xid * xid) * c1/xid - (6.0 * (3.0 - xid * xid) * s1) / sqr(xid) + Si1;
                    calI[l][3][1] = (24.0 - xid * xid) * c1 + (7.0 * xid * xid - 36.0) * s1 / xid;
                }
            }
			}
        }
        ...
        a=coeff->a1; b=coeff->b1; c=coeff->c1; d=coeff->d1; a2=coeff->a2; b2=coeff->b2; c2=coeff->c2; d2=coeff->d2;
        // we need to do xi<x1 and xi>x1 separately
		for (l=0;l<1;l++){
        if(xi<=xi1) {
            I0[i+l*Nxi] = (a+b*xi+c*xi*xi+d*xi*xi*xi)*(calI[l][0][2]-calI[l][0][0]) 
                - (b+2.0*c*xi+3.0*d*xi*xi)*(calI[l][1][2]-calI[l][1][0]) + (c+3.0*d*xi)*(calI[l][2][2]-calI[l][2][0])
                - d*(calI[l][3][2]-calI[l][3][0]);
            if (xi==0) I0[i+l*Nxi]=0;
        }
        else {
            I0[i+l*Nxi] = (a+b*xi+c*xi*xi+d*xi*xi*xi)*(calI[l][0][2]-calI[l][0][1]) 
                - (b+2.0*c*xi+3.0*d*xi*xi)*(calI[l][1][2]-calI[l][1][1]) + (c+3.0*d*xi)*(calI[l][2][2]-calI[l][2][1])
              - d*(calI[l][3][2]-calI[l][3][1]);
            //  I0 = (a+b*xi+c*xi*xi)*(calI[0][2]-calI[0][1]) 
             //    - (b+2.0*c*xi)*(calI[1][2]-calI[1][1]) + c*(calI[2][2]-calI[2][1]); 
            I0[i+l*Nxi] += (a2+b2*xi+c2*xi*xi+d2*xi*xi*xi)*(calI[l][0][1]-calI[l][0][0]) 
                - (b2+2.0*c2*xi+3.0*d2*xi*xi)*(calI[l][1][1]-calI[l][1][0])
             + (c2+3.0*d2*xi)*(calI[l][2][1]-calI[l][2][0]) - d2*(calI[l][3][1]-calI[l][3][0]);
        }
		}
    }
...
// Function to compute the six I0 values
int compute_I0_vectorized_sum(       // returns the six I0s for a given xi
    double *xi_list,         //   xi
    int Nxi,
    double xi1,         //   xi1
    double xi0,             //   xi0
...
// Function to compute the six I0 values
int compute_I0_vectorized(       // returns the six I0s for a given xi
    double *xi_list,         //   xi
    int Nxi,
    double xi1,         //   xi1
    double xi0,             //   xi0
    ...
    //    calI[n] = (double *)malloc(3 * sizeof(double));
    //}
    calI = (double ***)malloc(3 * sizeof(double **));
    for (l = 0; l < 3; l++) {
      calI[l] = (double **)malloc(4 * sizeof(double *));
      for (n = 0; n < 4; n++) {
        calI[l][n] = (double *)malloc(3 * sizeof(double));
...
// Function to compute the six I0 values
int compute_I0(       // returns the six I0s for a given xi
    double *xi_list,         //   xi
    int Nxi,
    double xi1,         //   xi1
    double xi0,             //   xi0
    ...
    if (type=='A'){
        if (l==0){
        calI[0][0] = 0.0;  calI[1][0] = -1.0;
        calI[2][0] = 0.0;  calI[3][0] = (2.0);
        }
        else if (l==1){
        //  l=1
        calI[0][0] = -1.0;  calI[1][0] = 0.0;
        calI[2][0] = -2.0;  calI[3][0] = 0.0;
        }
        else if (l==2){
        //  l=2
        calI[0][0] = 0.0;  calI[1][0] = -2.0;
        calI[2][0] = 0.0;  calI[3][0] = (-8.0);
        }
    }
    else if (type=='B'){
        if (l==0){
            ...
            calI[0][0] = 0.0;  calI[1][0] = -1.0;
            //calI[2][0] = 0.0;  calI[3][0] = -10.0;
            calI[2][0] = 0.0;  calI[3][0] = -6.0;
        }
        else if (l==1){
            //  l=4    this is actually j_1''
            calI[0][0] = 1.0/3.0;  calI[1][0] = 0.0;
            calI[2][0] = -2.0; calI[3][0] = 0.0;
        }
        else if (l==2){
            //  l=5    this is actually j_2''
    ...
    for (int i=0;i<Nxi;i++){
		...
		if (type=='A'){
            if (l==0){
                //  l=0
                calI[0][2] = Si0;  calI[1][2] = -c0;  
                calI[2][2] = -xi*c0+s0;  calI[3][2] = (2.0-xi*xi)*c0+2.0*xi*s0;
            }
            else if (l==1){
                //  l=1
                calI[0][2] = -s0/xi;  calI[1][2] = Si0-s0;
                calI[2][2] = -2.0*c0-xi*s0;  calI[3][2] = -3.0*xi*c0+(3.0-xi*xi)*s0;
            }
            else if (l==2){
                //  l=2
                calI[0][2] = (1.5/xi)*(c0-s0/xi)+Si0/2.0;  calI[1][2] = c0-3.0*s0/xi;
                calI[2][2] = xi*c0-4.0*s0+3.0*Si0;  calI[3][2] = (xi*xi-8.0)*c0-5.0*xi*s0;
            }
        }
        else if (type=='B'){
            if (l==0){
                //  l=3    this is actually j_0''
                calI[0][2] = c0/xi-s0/sqr(xi);  calI[1][2] = c0-2.0*s0/xi;
                calI[2][2] = xi*c0-3.0*s0+2.0*Si0;  calI[3][2] = (xi*xi-6.0)*c0-4.0*xi*s0;
            }
            else if (l==1){
                //  l=4    this is actually j_1''
                calI[0][2] = 2.0*c0/sqr(xi)+s0/xi*(1.0-2.0/sqr(xi));  calI[1][2] = 3.0*c0/xi+(xi*xi-3.0)*s0/sqr(xi);
                calI[2][2] =4.0*c0+(xi-6.0/xi)*s0;  calI[3][2] = 5.0*xi*c0-(11.0-xi*xi)*s0+6.0*Si0;
            }
            else if (l==2){
                //  l=5    this is actually j_2''
                calI[0][2] = (-(xi*(-9.0 + xi*xi)*c0) + (-9.0 + 4.0*xi*xi)*s0)/pow(xi,4.0);  
                calI[1][2] = ((12.0-xi*xi)*xi*c0+(5.0*xi*xi-12.0)*s0)/pow(xi,3.0);
                calI[2][2] = (18.0-xi*xi)*c0/xi-(6.0*(3.0-xi*xi)*s0)/sqr(xi)+Si0;
                calI[3][2] = (24.0-xi*xi)*c0+(7.0*xi*xi-36.0)*s0/xi;
            }
        }
        ...
        if (xi > xi1) {
            ...
            if (type=='A'){
                if (l==0){
                    ...
                    calI[1][1] = -c1;
                    calI[2][1] = -xid * c1 + s1;
                    calI[3][1] = (2.0 - xid * xid) * c1 + 2.0 * xid * s1;
                }
                else if (l==1){
                    //  l=1
                    calI[0][1] = -s1 / xid;
                    calI[1][1] = Si1 - s1;
                    calI[2][1] = -2.0 * c1 - xid * s1;
                    calI[3][1] = -3.0 * xid * c1 + (3.0 - xid * xid) * s1;
                }
                else if (l==2){
                    //  l=2
                    calI[0][1] = (1.5 / xid) * (c1 - s1 / xid) + Si1 / 2.0;
                    calI[1][1] = c1 - 3.0 * s1 / xid;
                    calI[2][1] = xid * c1 - 4.0 * s1 + 3.0 * Si1;
                    calI[3][1] = (xid * xid - 8.0) * c1 - 5.0 * xid * s1;
                }
            }
            else if (type=='B'){
                if (l==0){
                    //  l=3    this is actually j_0''
                    calI[0][1] = c1 / xid - s1 / sqr(xid);
                    calI[1][1] = c1 - 2.0 * s1 / xid;
                    calI[2][1] = xid * c1 - 3.0 * s1 + 2.0 * Si1;
                    calI[3][1] = (xid * xid - 6.0) * c1 - 4.0 * xid * s1;
                }
                else if (l==1){
                    //  l=4    this is actually j_1''
                    calI[0][1] = 2.0 * c1 / sqr(xid) + s1 / xid * (1.0 - 2.0 / sqr(xid));
                    calI[1][1] = 3.0 * c1 / xid + (xid * xid - 3.0) * s1 / sqr(xid);
                    calI[2][1] = 4.0 * c1 + (xid - 6.0 / xid) * s1;
                    calI[3][1] = 5.0 * xid * c1 - (11.0 - xid * xid) * s1 + 6.0 * Si1;
                }
                else if (l==2){
                    //  l=5    this is actually j_2''
                    calI[0][1] = (-(xid * (-9.0 + xid * xid) * c1) + (-9.0 + 4.0 * xid * xid) * s1) / pow(xid, 4.0);
                    calI[1][1] = ((12.0 - xid * xid) * xid * c1 + (5.0 * xid * xid - 12.0) * s1) / pow(xid, 3.0);
                    calI[2][1] = (18.0 - xid * xid) * c1/xid - (6.0 * (3.0 - xid * xid) * s1) / sqr(xid) + Si1;
                    calI[3][1] = (24.0 - xid * xid) * c1 + (7.0 * xid * xid - 36.0) * s1 / xid;
                }
            }
        }
        ...
        // we need to do xi<x1 and xi>x1 separately
        if(xi<=xi1) {
            I0[i] = (a+b*xi+c*xi*xi+d*xi*xi*xi)*(calI[0][2]-calI[0][0]) 
                - (b+2.0*c*xi+3.0*d*xi*xi)*(calI[1][2]-calI[1][0]) + (c+3.0*d*xi)*(calI[2][2]-calI[2][0])
                - d*(calI[3][2]-calI[3][0]);
            if (xi==0) I0[i]=0;
        }
        else {
            I0[i] = (a+b*xi+c*xi*xi+d*xi*xi*xi)*(calI[0][2]-calI[0][1]) 
                - (b+2.0*c*xi+3.0*d*xi*xi)*(calI[1][2]-calI[1][1]) + (c+3.0*d*xi)*(calI[2][2]-calI[2][1])
              - d*(calI[3][2]-calI[3][1]);
            //  I0 = (a+b*xi+c*xi*xi)*(calI[0][2]-calI[0][1]) 
             //    - (b+2.0*c*xi)*(calI[1][2]-calI[1][1]) + c*(calI[2][2]-calI[2][1]); 
            I0[i] += (a2+b2*xi+c2*xi*xi+d2*xi*xi*xi)*(calI[0][1]-calI[0][0]) 
                - (b2+2.0*c2*xi+3.0*d2*xi*xi)*(calI[1][1]-calI[1][0])
             + (c2+3.0*d2*xi)*(calI[2][1]-calI[2][0]) - d2*(calI[3][1]-calI[3][0]);
        }
    }
...
int compute_j0ffts(double x0,
    ...
    double j0,j1;
	double complex *j0_fft=kernels_fft[0][0];
	double complex *ddj0_fft=kernels_fft[1][0];
...
int compute_all_jffts(double x0,
    ...
    j0 = sin(x0)/x0;
    j1 = -cos(x0)/x0 + sin(x0)/sqr(x0);
    //j2 = (3.0/sqr(x0) - 1.0)*sin(x0)/x0 - 3.0*cos(x0)/cube(x0);
