...
int evolver_fss(int (*derivs)(double x,
				  double * y,
				  double * dy,
  ...
  struct generic_integrator_fss_workspace gi;
  double * dy;
  short call_output;
  ...
  class_call(initialize_generic_integrator_fss(y_size, &gi),
	     gi.error_message,
	     error_message);
  ...
  while ((x1 < x_end) && (next_index_x<x_size)) {
    ...
    if (x1 + 1.* timestep < x_sampling[next_index_x]) {
      x2 = x1 + timestep;
    }
    else {
...
//printf("%e %e %e %e %e %e\n",x1,timestep,x1 + 1.* timestep ,x_sampling[next_index_x],x2, x_end);
    ...
    class_call(generic_integrator_fss(derivs,
				  x1,
				  x2,
				  y,
				  parameters_and_workspace_for_derivs,
  ...
  class_call(cleanup_generic_integrator_fss(&gi),
	     gi.error_message,
	     error_message);
