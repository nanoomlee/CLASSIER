...
int array_interpolate_linear(
  ...
  if (x_array[inf] < x_array[sup]){
    ...
    if (x < x_array[inf]) {
      sprintf(errmsg,"%s(L:%d) : x=%e < x_min=%e",__func__,__LINE__,x,x_array[inf]);
      printf("%s(L:%d) : x=%e < x_min=%e, %.12e %.12e",__func__,__LINE__,x,x_array[inf], x, x_array[sup]);
      return _FAILURE_;
    }

    if (x > x_array[sup]) {
      sprintf(errmsg,"%s(L:%d) : x=%e > x_max=%e",__func__,__LINE__,x,x_array[sup]);
      printf("%s(L:%d) : x=%e > x_max=%e, %.20e %.20e",__func__,__LINE__,x,x_array[sup],x,x_array[sup]);
      return _FAILURE_;
    }
